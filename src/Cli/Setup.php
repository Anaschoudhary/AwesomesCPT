...
class Setup
{
	...
	public function __invoke(array $args, array $assoc_args): void
	{
		...
		// Replace in files
		if (
			!$this->replaceInFiles('awesome-plugin', $this->slug, ['.*\.php', '.*\.js', '.*\.json'])
			|| !$this->replaceInFiles('Awesome_Plugin', str_replace('-', '_', $this->slug), ['.*\.php'])
			|| !$this->replaceInFiles('Awesome_Plugin', $this->namespace, ['.*\.php', '.*\.json'])
			|| !$this->replaceInFiles('Awesome_Plugin', strtoupper($this->namespace), ['.*\.php', '.*\.json'])
			|| !$this->replaceInFiles('Awesome Plugin', $this->name, ['.*\.php', '.*README\.txt'])
		) {
	...
	{
		if (
			!rename($this->path . '/src/Awesome_Plugin.php', $this->path . "/src/$this->namespace.php")
			|| !rename($this->path . '/awesome-plugin.php', $this->path . "/$this->slug.php")
		) {
